...
def save_answers():
    ...
    # Process the user answers (data['userAnswers']) as needed
    # You can store them in a database, perform analysis, etc.

    # Example: Print user answers to the console
    print('Received User Answers:')
    for answer in data['userAnswers']:
        print(f'{answer["question"]}: {answer["answer"]}')

    return jsonify({'message': 'User answers received successfully'})
